...
def main():
    ...
    # Initialize ContactParticleFilter
    cpf = ContactParticleFilter(physics, config_path)

    # Initialize ResidualObserver
    ...
    test_measurement_model(cpf, residual_observer)
    test_run_contact_particle_filter(cpf, residual_observer)
...
def test_frame_conversions(cpf):
    ...
    assert np.allclose(cpf.particles_mesh_frame, mesh_frame, atol=1e-5), "Frame conversion error"
...
def test_measurement_model(cpf, residual_observer):
    print("\nTesting run_measurement_model()...")
    gamma_t, _ = residual_observer.get_residual(0)  # Assuming time=0
